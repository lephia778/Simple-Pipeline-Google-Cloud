...
default_args = {
    ...
    'start_date': days_ago(1), # ป้องกันไม่ให้ถูก catchup run ซ้ำย้อนหลัง
    'email': ['airflow@example.com'],
    'email_on_failure': False,
    'email_on_retry': False,
    'retries': 1,
    'retry_delay': timedelta(minutes=5),
    'schedule_interval': '@once',  # bulk load job
}

# Create DAG

dag = DAG(
    'online_retail_ws5',  # careful same id it will overwrite
...
t4 = BashOperator(
    ...
    bash_command='bq load --source_format=CSV --autodetect \
                    deworkshops.online_retail \
                    gs://australia-southeast1-sample-a12d86c5-bucket/data/result.csv',
    dag=dag
)
